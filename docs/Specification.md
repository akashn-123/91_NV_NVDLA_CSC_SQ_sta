\# Specification: To Understand RTL



NV\_NVDLA\_CSC\_sg (Convolution Sequence Generator)
...
\########To Understand how to read STA report



STA Timing Report Analysis \& Fixing Guide

Role

You are a digital design engineer analyzing a Static Timing Analysis (STA) report for a synchronous digital circuit. Your goal is to understand the timing violation and reason through possible fixes without jumping to solutions.
...
Startpoint / Endpoint: The launch and capture flip-flops bounding the timing path.

Path Group / Path Type: max means this is a setup (hold is min) timing check — the data must arrive before the clock edge captures it.



Column Definitions

ColumnMeaning: Cap-Capacitive load (fF or fF-equivalent) driven by the cell output at that node

Slew-Signal transition time (rise or fall). Higher slew = slower signal edge

Delay-Incremental delay added by this cell or net

Time-Cumulative arrival time from the startpoint

^ / v-Rising or falling transition at this point in the path
...
      = (Clock Period − Setup Time) − Path Delay

A negative slack (VIOLATED) means the data arrives too late for the capture flip-flop to reliably sample it.



Key Physics to Understand Before Fixing

1\. Capacitance and Drive Strength



Every cell's output drives a load capacitance made up of net parasitic cap + input pin caps of fanout cells.

Increasing drive strength of a cell lowers its own output resistance, reducing its own delay — but it also increases its input pin capacitance, which loads the previous stage more heavily, potentially increasing the delay of the preceding cell. This is a fundamental tradeoff.

Always consider the upstream impact when upsizing a cell.
...
Slew (transition time) accumulates and degrades as a signal propagates through the netlist. A slow input slew into a cell causes a slow output slew, which then causes high delay in the next cell.

A high Slew value at any row is a signal that the preceding driver is undersized or driving excessive capacitance.

Fixing slew at the source of degradation is more effective than treating downstream symptoms.
...
A net with many fanout pins accumulates large total capacitive load. This manifests as a large Cap value and typically a high Slew.

These nets often need buffering — inserting intermediate drivers to split the load — rather than simply upsizing the driving cell.



4\. Logic Depth and Gate Topology



Some gate types are inherently slower due to series transistor stacking (e.g., NOR4 is slower than a chain of NOR2s in CMOS).

Complex gates like AOI222, OAI221 often have high intrinsic delay and can be restructured.

The number of logic levels between flip-flops directly limits the achievable clock frequency.
...
If the combinational logic depth is fundamentally too large to fit within one clock period at the target frequency, adding pipeline registers splits the path into shorter sub-paths, each meeting timing individually.

Potential pipeline insertion points in a path are typically at locations where:



There is a natural functional boundary or intermediate result

The cumulative delay at that node is approximately a multiple of the target clock period

The signal is not part of a feedback loop that would be broken by a cycle of latency
...
Every fix has a cost: area, power, or a new timing problem elsewhere.

A fix that closes timing on one path may open a violation on another (e.g., adding a buffer adds latency on a different arc).

Pipelining changes functional latency — this is acceptable in some architectures and breaks others.

The goal is to reason about tradeoffs, not to apply fixes mechanically.
